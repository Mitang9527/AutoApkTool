...
from .constants import (
    PATH_SLCLIENT_JSON, LOGIN_TYPE_MAPPING, MAP_CONFIG_TEMPLATES,
    LBS_COOR_PATH, LBS_MAP_PYPE, ANDROID_NAMESPACE, PATH_MANIFEST_XML
)
...
def modify_manifest(is_enabled: bool, log_callback=None):
    ...
    if not os.path.exists(manifest_path):
        if log_callback: log_callback(" Error: Unable to find AndroidManifest.xml")
        return
    ...
    try:
        ...
        if application is None:
            if log_callback: log_callback(" Error: Unable to find the <application>tag")
            return
        ...
        if target_activity is None:
            if log_callback: log_callback(f" Error: Target Activity not found (candidate:{candidate_activity_names})")
            return

        intent_filter = target_activity.find("intent-filter")
        if intent_filter is None:
            if log_callback: log_callback(f" Error：{found_activity_name} No<intent-filter>, cannot operate")
            return
        ...
        if is_enabled:
            if not has_home:
                ...
                if log_callback: log_callback(f"[ok]  set to desktop as Launcher\n")
        else:
            if has_home:
                intent_filter.remove(home_category_elem)
                write_pretty_xml(tree, manifest_path)
                if log_callback: log_callback(f"[ok] Launcher has  cancelled\n")

    except Exception as e:
        if log_callback: log_callback(f" operation failed：{e}")
...
def update_version_info(yml_path: Path, log_callback=None) -> None:
    ...
    if not old_code or not old_name:
        if log_callback: log_callback("[Warn]Unable to obtain version information, skipping update\n")
        return
